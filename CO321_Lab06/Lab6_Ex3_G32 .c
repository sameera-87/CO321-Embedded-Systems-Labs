...
char caesar_encrypt(char ch, int key) {
    if (ch >= 'a' && ch <= 'z') {
        return ((ch - 'a' + key) % 26) + 'a'; // to wrap around the alphabet (so after 'z', it goes back to 'a').
    } else if (ch >= 'A' && ch <= 'Z') {
...
int main(void) {
    usart_init();   //Initialize USART communication at 9600 baud

    char buffer[100];     // To store received input string
    char encrypted[100];  // To store encrypted output string
    int i = 0;
    char ch;

    while (1) {
        // Receive until carriage return (\r or '\n')
        i = 0;
        while (1) {
            ch = usart_receive();  // Receive a single character over USART
            if (ch == '\r' || ch == '\n') {
                break;
            }
            buffer[i++] = ch;
        ...
        // Apply Caesar Cipher (key = 3)
        int j;
        for (j = 0; j < i; j++) {
            encrypted[j] = caesar_encrypt(buffer[j], 3);
        }
        encrypted[i] = '\0';
